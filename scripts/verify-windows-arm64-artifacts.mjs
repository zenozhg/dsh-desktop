...
export async function verifyWindowsArm64Artifacts(options = {}) {
  ...
  const unpackedDir = resolve(options.unpackedDir ?? join(releaseDir, 'win-unpacked'))
  const installer = await findArm64Installer(releaseDir)

  const unpackedStat = await stat(unpackedDir).catch(() => undefined)
  if (!unpackedStat?.isDirectory()) {
    throw new Error(`Expected unpacked directory for ARM64 verification: ${unpackedDir}`)
  }
